...
describe("TelegramBot", () => {
	...
	describe("editMessageText", () => {
		it("should edit a message and return true on success", async () => {
			mockFetchSuccess({ ok: true });

			const result = await bot.editMessageText(123, "edited text", {
				reply_markup: { inline_keyboard: [] },
			});

			expect(globalThis.fetch).toHaveBeenCalledWith(
				`https://api.telegram.org/bot${mockToken}/editMessageText`,
				expect.objectContaining({
					method: "POST",
					body: JSON.stringify({
						chat_id: mockSupergroupId,
