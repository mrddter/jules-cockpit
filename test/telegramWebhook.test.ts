...
describe('telegramWebhookHandler (Phase 5)', () => {
  ...
  beforeEach(() => {
    ...
    mockBind = vi.fn().mockReturnValue({ run: mockRun, all: mockAll, first: mockFirst });
  ...
  const createCtx = () => {
    ...
    return {
      waitUntil: (p: Promise<void>) => { _waitUntilPromise = p; return p; },
      passThroughOnException: () => {},
      _getWaitUntilPromise: () => _waitUntilPromise
    } as unknown as ExecutionContext & { _getWaitUntilPromise: () => Promise<void> | undefined };
  };
  ...
  it('TSK-5.1: should ignore messages without message_thread_id', async () => {
    ...
    expect(mockPrepare).not.toHaveBeenCalledWith('SELECT repo_name FROM repositories WHERE telegram_topic_id = ?');
  });
  ...
  it('TSK-5.1: should ignore messages in unknown topics', async () => {
    ...
    expect(mockPrepare).toHaveBeenCalledWith('SELECT repo_name FROM repositories WHERE telegram_topic_id = ?');
  ...
  it('TSK-5.2: should handle /new command and create session', async () => {
    ...
    expect(mockPrepare).toHaveBeenCalledWith('INSERT INTO sessions (jules_session_id, telegram_topic_id, status, created_at, updated_at) VALUES (?, ?, ?, ?, ?)');
  ...
  it('TSK-5.2: should handle /close command', async () => {
    ...
    expect(mockPrepare).toHaveBeenCalledWith('UPDATE sessions SET status = ?, updated_at = ? WHERE jules_session_id = ?');
    expect(mockBind).toHaveBeenCalledWith('archived', expect.any(String), 'sess-1');
    expect(mockRun).toHaveBeenCalled();
  });
  ...
  it('TSK-5.3: should forward non-command messages to active session', async () => {
    ...
    expect(mockSendUserMessage).toHaveBeenCalledWith('sess-1', 'just a normal message');
  });
});
