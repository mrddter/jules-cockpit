...
describe('JulesClient', () => {
  ...
  const mockFetchError = (status: number, statusText: string, errorData: unknown) => {
    (globalThis.fetch as ReturnType<typeof vi.fn>).mockResolvedValue({
      ok: false,
      status,
      statusText,
  ...
  describe('createSession', () => {
    ...
    it('should throw an error if API request fails', async () => {
      ...
      await expect(client.createSession('source', 'prompt')).rejects.toThrowError(
  ...
  describe('listActivities', () => {
    it('should send correct GET request to list activities', async () => {
      const mockActivities = [
        { type: 'agentMessaged', agentMessage: 'hi' },
      ];
